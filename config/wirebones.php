...
return [
    ...
    | The package default is storage/, which assumes deployments re-run the
    | build. Capturing needs Playwright's Chromium, which is a local-only dev
    | dependency here, so the generated files are committed and shipped with
    | the release instead — and therefore have to live somewhere tracked that
    | is part of the release, which storage/ is not.
    |
    | Re-run `php artisan wirebones:build` and commit the result whenever a
    | dashboard widget's layout changes.
    |
    */

    'output_path' => resource_path('wirebones'),
    ...
    | rendering stays cheap. These are the committed artifacts: `view:cache`
    | only Blade-compiles them, so a deploy needs neither Chromium nor a
    | running app.
    |
    */

    'compiled_path' => resource_path('wirebones/views'),
